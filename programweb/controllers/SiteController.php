...
//                ]);
        ...
        if ($model->load(Yii::$app->request->post()) && $model->login()) {
            $alumno = $model->getUser()->getAlumno()->one();
            if ($alumno) {
                return $this->redirect("?r=alumno/view&id=".$alumno->idalumno);
            }
            else{
                $profesor = $model->getUser()->getProfesor()->one();
                if ($profesor) {
                    return $this->redirect("?r=profesor/view&id=".$profesor->idProfesor);
                }
            }
        }
    ...
    function multichain($method) { // other params read from func_get_args()
        ...
        return json_rpc_send($multichain_chain['rpchost'], $multichain_chain['rpcport'], $multichain_chain['rpcuser'], $multichain_chain['rpcpassword'], $method, array_slice($args, 1));
    }
    ...
    function multichain_with_raw(&$rawresponse, $method) { // other params read from func_get_args()
        ...
        return json_rpc_send($multichain_chain['rpchost'], $multichain_chain['rpcport'], $multichain_chain['rpcuser'], $multichain_chain['rpcpassword'], $method, array_slice($args, 2), $rawresponse);
    }
        ...
        if (!isset($multichain_max_data_size))
            if (no_displayed_error_result($params, multichain('getblockchainparams')))
                $multichain_max_data_size = min(
                        $params['maximum-block-size'] - 80 - 320, $params['max-std-tx-size'] - 320, $params['max-std-op-return-size']
                );
